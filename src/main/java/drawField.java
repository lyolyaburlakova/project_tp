...
public class drawField extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        g.drawString("Поле кораблей", 210, 535);
        g.drawString("Поле выстрелов", 810, 535);

        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 10; j++) {
                g.drawString(game.player1fieldwithships[i][j] + "", 210, 535+(i+1)*10);
